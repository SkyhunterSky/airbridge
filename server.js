...
function onWsMessage(socket, text) {
  ...
  if (msg.type === 'join') {
    ...
    // Once both sides are present, tell EVERY socket in the room at once
    // (including the one that just joined) so sender and receiver see a
    // synchronized "connected" confirmation instead of each guessing based
    // on local, one-sided state.
    if (set.size === 2) {
      wsBroadcast(code, { type: 'both-connected' }, null);
    }
